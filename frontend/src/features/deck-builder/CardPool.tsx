...
export default function CardPool({ leader, getQuantity, onAddCard, onCardSelect }: Props) {
  ...
  const inputClass =
    'bg-surface-1 text-text-primary rounded px-2.5 py-1.5 text-xs border border-glass-border focus:outline-none focus:ring-1 focus:ring-op-ocean/30 focus:border-op-ocean';
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden glass rounded-[var(--radius-glass)]">
      {/* Mode Toggle + Filters */}
      <div className="shrink-0 border-b border-glass-border bg-surface-2/80 backdrop-blur-sm px-3 py-2 space-y-2">
        {/* Mode Toggle */}
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex gap-0.5 shrink-0">
            <button
              onClick={() => { setMode('suggested'); setPage(0); }}
              className={`px-3 py-1 rounded-l text-xs border border-glass-border cursor-pointer ${
                mode === 'suggested' ? 'bg-op-ocean text-white' : 'bg-surface-2 text-text-secondary hover:text-text-primary'
              }`}
            >
              Suggested
            </button>
            <button
              ...
              className={`px-3 py-1 rounded-r text-xs border border-glass-border cursor-pointer ${
                mode === 'browse' ? 'bg-op-ocean text-white' : 'bg-surface-2 text-text-secondary hover:text-text-primary'
              }`}
            >
              Browse All
            </button>
          ...
          {mode === 'suggested' && !leader && (
            <span className="text-xs text-text-muted italic">Select a leader to see suggestions</span>
          )}

          {mode === 'browse' && (
            <div className="contents animate-fade-up">
              <input
                type="text"
              ...
              <div className="flex items-center gap-1 shrink-0">
                ...
                <input type="number" placeholder="Max" value={costMax} onChange={(e) => { setCostMax(e.target.value); setPage(0); }} className={`${inputClass} w-12`} min={0} />
              </div>
              ...
              <button
                ...
                className={`px-2 py-1 rounded text-xs border border-glass-border cursor-pointer shrink-0 ${
                  sortOrder === 'asc' ? 'bg-op-ocean text-white' : 'bg-surface-2 text-text-secondary'
                }`}
              >
                {sortOrder === 'asc' ? 'ASC' : 'DESC'}
              </button>
              <Button variant="ghost" size="sm" onClick={handleFilterReset} className="shrink-0">
                Reset
              </Button>
            </div>
          )}
      ...
      {/* Card Grid */}
      <div key={mode} className="flex-1 overflow-auto p-4 page-enter">
        {loading ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            {Array.from({ length: 15 }).map((_, i) => (
              <div key={i} className="bg-surface-2 rounded-lg overflow-hidden animate-pulse">
            ...
            {cards.map((card) => {
              ...
              return (
                <div
                  ...
                  className={`bg-surface-2 rounded-lg overflow-hidden cursor-pointer transition-all hover:scale-105 relative ${
                    maxed
                      ? 'opacity-40 ring-1 ring-glass-border'
                      : qty > 0
                        ? 'ring-2 ring-op-ocean/50 hover:ring-op-ocean'
                        : 'hover:ring-2 hover:ring-op-ocean'
                  }`}
                >
                  {/* Quantity badge */}
                  {qty > 0 && (
                    <div className={`absolute top-1.5 right-1.5 z-10 w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shadow-lg ${
                      maxed ? 'bg-surface-3 text-text-secondary' : 'bg-op-ocean text-white'
                    }`}>
                      {qty}
                    </div>
                  )}
                  ...
                  {/* Card Info */}
                  <div className="p-2">
                    <p className="text-xs text-text-primary truncate" title={card.name}>{card.name}</p>
                    <div className="flex items-center gap-1.5 mt-1">
                      {card.cost !== null && (
                        <span className="text-[10px] bg-surface-3 rounded px-1.5 py-0.5 text-text-secondary">
                          {card.cost}
                        </span>
                      )}
                      {colors.map((c) => (
      ...
      {mode === 'browse' && total > PAGE_SIZE && (
        <div className="shrink-0 border-t border-glass-border glass px-4 py-2 flex items-center justify-between text-xs">
