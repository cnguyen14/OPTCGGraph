...
export default function DeckPanel({
  ...
  return (
    <div className="w-[380px] shrink-0 flex flex-col glass overflow-hidden">
      {/* Header */}
      <div className="px-4 pt-3 pb-2 border-b border-glass-border">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-text-primary font-semibold text-sm">Deck</h3>
          <div className="flex items-center gap-2">
            <span className="text-xs text-text-muted">
              {totalCards}/50 &middot; ${totalPrice.toFixed(2)}
            </span>
            {leader && totalCards >= 10 && (
              <Button variant="success" size="sm" onClick={() => setShowValidation(true)} className="text-[11px]">
                Validate
              </Button>
            )}
          </div>
        ...
        {Object.keys(typeCount).length > 0 && (
          <div className="flex gap-3 mt-2">
            {Object.entries(typeCount)
              .sort(([, a], [, b]) => b - a)
              .map(([type, count]) => (
                <span key={type} className="text-[10px] text-text-muted">
                  {type}: <span className="text-text-secondary">{count}</span>
                </span>
              ))}
          </div>
        )}
