  ...
  return (
    <GlassCard className="p-4">
        ...
        <div className="flex gap-1">
          {games.map((g, i) => (
            <Button
              ...
              variant={activeGame === i ? 'primary' : 'ghost'}
              size="sm"
              className="text-[11px]"
            >
              Game {i + 1}
      ...
      <div className="flex items-center gap-4 mb-3 text-[11px]">
        <span className={game.winner === 'p1' ? 'text-blue-400 font-bold' : 'text-text-secondary'}>
          P1 ({game.p1_life} life)
        </span>
        <span className="text-text-muted">vs</span>
        <span className={game.winner === 'p2' ? 'text-red-400 font-bold' : 'text-text-secondary'}>
          P2 ({game.p2_life} life)
        </span>
        <span className="text-text-muted ml-auto">{game.turns} turns</span>
      </div>
...
function ReplayLine({ event }: { event: GameReplayEntry }) {
  ...
  return (
    <div className="flex gap-2 py-0.5 hover:bg-surface-2 px-1 rounded">
      <span className="text-text-muted w-8 shrink-0">T{event.turn}</span>
