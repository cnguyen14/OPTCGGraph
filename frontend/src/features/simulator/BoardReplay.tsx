...
export default function BoardReplay({ gameLog, p1Leader, p2Leader, winner }: BoardReplayProps) {
  ...
  return (
    <div className="glass-heavy overflow-hidden">
      {/* Controls bar */}
      <div className="flex items-center gap-3 px-4 py-2.5 bg-surface-2 border-b border-glass-border">
        {/* Transport controls */}
        <div className="flex items-center gap-1">
          <button
            onClick={() => { setCurrentStep(0); setIsPlaying(false); }}
            className="px-2 py-1 text-xs text-text-secondary hover:text-text-primary hover:bg-surface-3 rounded transition-colors font-mono"
            title="Go to start"
          >
            |&lt;
          </button>
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          />
          <span className="text-[10px] text-text-muted font-mono w-8">{speed.toFixed(1)}s</span>
        </div>

        {/* Turn indicator */}
        <div className="flex items-center gap-2 shrink-0">
          <span className="text-xs text-text-muted">Turn {state.turn}</span>
      ...
      {/* Progress scrubber */}
      <div className="px-4 py-1 bg-surface-1">
        <input
          type="range"
          min={0}
          max={totalSteps - 1}
