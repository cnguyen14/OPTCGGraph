  ...
  return (
    <GlassCard variant="subtle" className="p-6">
      ...
      <div className="h-3 bg-surface-2 rounded-full overflow-hidden mb-3">
        <div
          className="h-full bg-gradient-to-r from-op-ocean to-purple-500 rounded-full transition-all duration-300"
          style={{ width: `${pct}%` }}
        />
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 text-center">
        <div>
          ...
          <div className="text-[11px] text-text-secondary mt-1">{p1Leader || 'Player 1'}</div>
          {progress.completed > 0 && (
            <div className="text-[10px] text-blue-400/60 mt-0.5">{p1Rate}%</div>
          )}
        </div>
        <div>
          <div className="text-2xl font-bold text-text-muted">{progress.draws}</div>
          <div className="text-[11px] text-text-secondary mt-1">Draws</div>
        </div>
        <div>
          <div className="text-2xl font-bold text-red-400">{progress.p2Wins}</div>
          <div className="text-[11px] text-text-secondary mt-1">{p2Leader || 'Player 2'}</div>
          {progress.completed > 0 && (
