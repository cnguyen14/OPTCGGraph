    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-gray-700/50 bg-gray-900/50 p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-white">Card Performance</h3>
          <div className="flex gap-1">
            <button
              onClick={() => setSortBy('played')}
              className={`text-[11px] px-2 py-1 rounded ${sortBy === 'played' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white'}`}
            >
              Most Played
            </button>
            <button
              onClick={() => setSortBy('winrate')}
              className={`text-[11px] px-2 py-1 rounded ${sortBy === 'winrate' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white'}`}
            >
              Win Rate
            </button>
          </div>
        ...
        <div className="space-y-1 max-h-64 overflow-y-auto">
          {cardStats.slice(0, 20).map((card) => {
            ...
            return (
              <div
                key={card.card_id}
                className="flex items-center gap-3 py-1.5 px-2 rounded hover:bg-gray-800/50"
              >
          ...
          {cardStats.length === 0 && (
            <p className="text-xs text-gray-500 text-center py-4">No card data available</p>
          )}
        </div>
      </div>
    </div>
...
function StatCard({
  ...
  return (
    <div className={`rounded-lg border p-3 ${colorClasses[color]}`}>
      ...
      <div className="text-[10px] text-gray-500 mt-1 truncate">{sublabel}</div>
    </div>
  );
}
