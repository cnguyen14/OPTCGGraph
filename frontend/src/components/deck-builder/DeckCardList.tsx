          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1 mt-0.5">
                {card.cost !== null && (
                  <span className="text-[9px] text-gray-500">{card.cost}</span>
                )}
                {colors.map((c) => (
                  <span
                    key={c}
            ...
            <div className="flex items-center gap-1 shrink-0" onClick={(e) => e.stopPropagation()}>
              <button
                ...
                className="w-5 h-5 rounded text-[10px] bg-gray-700 hover:bg-red-600 text-gray-400 hover:text-white transition-colors flex items-center justify-center"
              >
                -
              </button>
              <span className="text-xs text-white w-4 text-center font-medium">{quantity}</span>
              <button
                onClick={() => onAdd(card)}
                className="w-5 h-5 rounded text-[10px] bg-gray-700 hover:bg-blue-600 text-gray-400 hover:text-white transition-colors flex items-center justify-center disabled:opacity-30 disabled:cursor-not-allowed"
                disabled={quantity >= 4}
              >
                +
              </button>
