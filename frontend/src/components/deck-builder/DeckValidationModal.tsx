...
export default function DeckValidationModal({ open, leader, entries, onApply, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div
        className="bg-gray-900 border border-gray-700 rounded-xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-hidden flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="px-6 py-4 border-b border-gray-800 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-bold text-white">Deck Validation</h2>
            <p className="text-xs text-gray-400">{leader.name} ({leader.id}) &middot; {cardIds.length} cards</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          ...
          {phase === 'validating' && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-400">Validating deck...</p>
            </div>
          )}
          ...
          {phase === 'suggestions' && suggestions.length > 0 && (
            <div className="space-y-3 mt-4">
              <h3 className="text-white font-semibold text-sm">Suggested Replacements</h3>
              <p className="text-xs text-gray-500">Select which swaps to apply. Rule fixes are auto-selected.</p>
              ...
              {suggestions.map((s, i) => (
                <label
                  ...
                  className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${
                    selected.has(i)
                      ? s.priority === 'high' ? 'bg-red-900/20 border-red-700' : 'bg-blue-900/20 border-blue-700'
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`text-[10px] font-bold uppercase px-1.5 py-0.5 rounded ${
                        s.priority === 'high' ? 'bg-red-900 text-red-300' : s.priority === 'medium' ? 'bg-yellow-900 text-yellow-300' : 'bg-gray-700 text-gray-300'
                      }`}>
                        {s.priority}
                      </span>
                      <span className="text-xs text-gray-500">{s.check_name}</span>
                    </div>
                    ...
                    <div className="grid grid-cols-[1fr,auto,1fr] items-center gap-2 text-sm">
                      ...
                      <div className="bg-red-900/20 rounded p-2">
                        ...
                        <div className="text-gray-500 text-xs mt-0.5">{s.remove.reason}</div>
                      </div>
                      ...
                      <div className="bg-green-900/20 rounded p-2">
                        ...
                        <div className="text-gray-500 text-xs mt-0.5">{s.add.benefit}</div>
                      </div>
                    </div>
                  </div>
                </label>
          ...
          {phase === 'suggestions' && suggestions.length === 0 && (
            <p className="text-gray-400 text-sm text-center py-4">No suggestions available — deck looks good!</p>
          )}
          ...
          {phase === 'applying' && (
            <div className="text-center py-8">
              <div className="animate-spin w-8 h-8 border-2 border-blue-500 border-t-transparent rounded-full mx-auto mb-3" />
              <p className="text-gray-400">Applying changes and re-validating...</p>
            </div>
          )}
