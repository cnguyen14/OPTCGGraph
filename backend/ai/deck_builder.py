...
def _fill_deck(
    ...
    """Fill a 50-card deck using strategy template with role-based slot allocation."""
    deck: list[dict] = []
    ...
    # Phase 1: Fill essential roles (prioritize by relevance within each role)
    role_targets = template["role_targets"]
    for role, target_count in role_targets.items():
        pool = sorted(categorized.get(role, []), key=lambda c: -c.get("relevance", 0))
        added = 0
        for card in pool:
    ...
    # Phase 2: Fill by cost curve targets
    cost_targets = template["cost_targets"]
    for (cost_min, cost_max), (target_min, target_max) in cost_targets.items():
        current = sum(1 for c in deck if cost_min <= (c.get("cost") or 0) <= cost_max)
        if current < target_min:
            needed = target_min - current
            pool = [c for c in candidates if cost_min <= (c.get("cost") or 0) <= cost_max]
            pool.sort(key=lambda c: (-c.get("relevance", 0), -(c.get("counter") or 0)))
            for card in pool:
    ...
    # Phase 3: Fill remaining slots to reach 50
    if len(deck) < 50:
        # Prefer high-counter cards and high-relevance
        remaining = [c for c in candidates if can_add(c)]
        remaining.sort(key=lambda c: (-(c.get("counter") or 0), -c.get("relevance", 0)))
        for card in remaining:
            if len(deck) >= 50:
                break
            add_card(card)

    # Phase 4: Ensure we have exactly 50 by adding copies of good cards
    if len(deck) < 50:
        # Add more copies of cards already in deck (up to 4x)
        existing = [c for c in deck]
        for card in sorted(existing, key=lambda c: -c.get("relevance", 0)):
            if len(deck) >= 50:
                break
            if deck_ids[card["id"]] < 4:
                add_card(card)

    # Trim to 50 if somehow over
    deck = deck[:50]
